...
<body>
	...
	<script>
		...
		$(function(){	
			<#if passiveMessage?has_content>
				$("input[name='passiveMessageKey']").val("${passiveMessage.passiveMessageKey}");
			<#else>
				$("input[name='passiveMessageKey']").val("");
			</#if>
			<#if news?has_content>
				if(${news.newsType} == 2){
			...
			$("#saveOrUpdateMessage").click(function() {
				...
				if(passiveMessageKey.length == 0 || passiveMessageKey == undefined){
					$("input[name='passiveMessageKey']").after("<span class='tip'>关键词不能为空</span>")
					return;
				}
				//文本为2.单图文多图文都为1
				...
				if ($(".bar li.sel").hasClass("news")) {
					...
					if(content == undefined || content == ""){
						$(".content").after("<span class='tip'>请输入回复内容</span>")
						return;
					}
				}
